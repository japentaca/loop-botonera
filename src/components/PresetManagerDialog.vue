<template>
  <Dialog v-model:visible="presetStore.isDialogOpen" modal header="Gestión de Presets"
    :style="{ width: '90vw', maxWidth: '1200px' }" :closable="true" @hide="closeDialog" class="preset-manager-dialog">
    ...
    <div class="toolbar">
      <div class="search-section">
        <input v-model="searchQuery" type="text" placeholder="Buscar presets..." class="search-input" />
      </div>
    ...
    <div class="presets-table-container">
      <table class="presets-table">
        ...
        <tbody>
          <tr v-for="preset in filteredAndSortedPresets" :key="preset.id"
            ...
            <td class="preset-name-cell">
              ...
              <input v-else v-model="editingName" @blur="finishEditing" @keyup.enter="finishEditing"
                @keyup.escape="cancelEditing" class="edit-input" ref="editInput" />
              <span v-if="preset.id === presetStore.currentPresetId" class="current-indicator" title="Preset actual">
                ★
              </span>
            </td>
            <td @dblclick="preset.id !== presetStore.currentPresetId ? loadPresetAndClose(preset.id) : null"
              style="cursor: pointer;" title="Doble click para cargar preset">
            ...
            <td class="actions-cell">
              <button @click="loadPreset(preset.id)" :disabled="preset.id === presetStore.currentPresetId"
                class="btn btn-small btn-primary" title="Cargar preset">
              ...
              <button @click="duplicatePreset(preset.id)" class="btn btn-small btn-secondary" title="Duplicar preset">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect>
                  <path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path>
                </svg>
              </button>
              <button @click="startEditing(preset)" class="btn btn-small btn-secondary" title="Renombrar preset">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                  <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                </svg>
              </button>
              <button @click="confirmDelete(preset)" class="btn btn-small btn-danger" title="Eliminar preset">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div v-if="showCreateForm" class="create-form-overlay" @click="showCreateForm = false">
      <div class="create-form" @click.stop>
        ...
        <input v-model="newPresetName" type="text" placeholder="Nombre del preset" class="form-input"
          @keyup.enter="createPreset" @keyup.escape="showCreateForm = false" ref="createInput" />
